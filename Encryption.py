...
numbersngenerated from Logistic system
...
# Using logistic_key function to generate 3 lists of keys
xkey, ykey, zkey = key.logistic_key(0.01, 0.02, 0.03, height*width)
...
for i in range(height):
    k = 0
    for j in range(width):
        decryptedImage[i][xindex[k]] = decryptedImage[i][j]
        k += 1
